...
public class InstructorService {
	...
	// 3-2. 배점관리 - 배점삭제 가능한 리스트만 출력하기 성적(&시험) 정보가 포함된 과목 리스트 보고난 후 배점 삭제할 과목 선택하고 배점
	// 삭제하기
	...
	private void menu_40(Scanner sc, String action) {
		...
		System.out.println(
				"---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		System.out.println(
				"과목코드 / 과목명 / 과목시작일 / 과목종료일 / 교재명 / 과정코드 / 과정명 / 과정시작일 / 과정종료일 / 강의실 / 시험코드 / 시험날짜 / 출결배점 / 필기배점 / 실기배점 / 성적 등록 인원");
		System.out.println(
				"---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");

		for (Instructor i : subjectListByNumberOfStudent) {
			System.out.printf("%s / %s / %s / %s / %s / %s / %s / %s / %s / %s / %s / %s / %d / %d / %d / %d%n",
					i.getOpenSubCode(), i.getSubjectName(), i.getOpenSubStartDate(), i.getOpenSubCloseDate(),
					i.getBookName(), i.getOpenCocode(), i.getCourseName(), i.getOpenCoStartDate(),
					i.getOpenCoCloseDate(), i.getClassName(), i.getTestCode(), i.getTestDate(),
					i.getAttendanceDistribution(), i.getWritingDistribution(), i.getPracticeDistribution(),
					i.getNumberOfStuHaveScore());
		}
		...
		System.out.printf("성적을 %s할 과목코드를 입력해 주세요 > ", actionInKorean);

		String openSubCode = sc.nextLine();

		this.menu_400(sc, openSubCode, action);

	}

	// 4-1 & 4-2 성적 등록 / 삭제할 학생 선택할 학생정보 리스트도 동일
	private void menu_400(Scanner sc, String openSubCode, String action) {

		List<Instructor> scoreInfoByStudents = this.insDAO.scoreInfoByStudents(openSubCode);
		...
		for (Instructor i : scoreInfoByStudents) {
			...
			System.out.printf("%s / %s / %s / %s / %s / %d / %d / %d / %d%n", i.getMid(), i.getName_(), i.getPhone(),
					...
					i.getPracticeScore(), i.getTotalScore());
	...
	private void menu_42(Scanner sc) {
		...
		if (yesOrNo.equalsIgnoreCase("y")) {
			...
			int result = this.insDAO.deleteScore(mid);
